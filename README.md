...
所有用户消息统一进入队列，由一个后台 Worker 串行处理。即使多个用户同时发消息，或同一用户连续发送多条消息，Claude 的查询也会依次执行，不会出现并发冲突。

### 回复链（Threaded Reply）

Claude 的每条回复都作为对用户消息的**引用回复**发出，而不是单独发送一条消息。对话结构清晰，可追溯。

### 引用消息感知

当用户在飞书中**引用某条消息**并发送新消息时，Claude 能够感知到被引用的原文，以 `[引用消息: id] 发送者: 内容` 的格式注入到 prompt 中。

### 音频消息支持

用户发送语音消息时，机器人将其下载保存为 `.opus` 文件，以 `[Audio: /path/to/file.opus (xs)]` 的格式传给 Claude 处理。

### 实时流式推送

Claude 生成回复时，文字内容**实时推送**到飞书聊天中（带缓冲机制），工具调用时立即 flush。回复完成后不再重复发送完整内容，避免重复刷屏。
...
- `/feishu auth` — 发起飞书授权流程，授权后机器人拥有上传文件等用户级权限
- `/feishu` — 查看所有可用命令
