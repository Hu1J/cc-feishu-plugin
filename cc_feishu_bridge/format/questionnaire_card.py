...
class _AskUserQuestionMarker:
    ...
    def __init__(self, tool_name: str, tool_input: str):
        ...
        if parsed is not None:
            self.card = self._build_card(parsed)
        else:
            self.card = None

    def _build_card(self, data: _QuestionnaireData) -> dict:
        elements = []
...
def format_questionnaire_card(marker: "_AskUserQuestionMarker") -> dict:
    ...
    if marker.card is None:
        raise ValueError("AskUserQuestionMarker had invalid input")
    return marker.card